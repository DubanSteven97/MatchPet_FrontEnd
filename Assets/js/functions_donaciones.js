...
document.addEventListener('DOMContentLoaded',function(){
	...
	tableOrganizaciones = $('#tableProcesosAdopcion').dataTable({
		...
		"ajax":{
			"url":""+BaseUrl+"/Donaciones/GetDonaciones",
			"dataSrc":""
		},
		"columns": [
            { "data": 'idDonacion' },
	...
	let formProcesoAdopcion = document.querySelector("#formProcesoAdopcion");
    formProcesoAdopcion.onsubmit = function(e){
        e.preventDefault();
        ...
        let ajaxUrl = BaseUrl+'/Donaciones/gestionProceso';
        ...
        request.onreadystatechange = function(){
            if(request.readyState == 4 && request.status == 200){
                ...
                if(objData.estado)
                {
					...
					switch (intStatus) {
						case 1:
							htmlStatus = '<span class="badge badge-primary">En proceso</span>';
							break;
						case 2:
							htmlStatus = '<span class="badge badge-danger">Inactivo</span>';
							break;
						case 3:
							htmlStatus = '<span class="badge badge-success">Aprobado</span>';
							break;
						case 4:
							htmlStatus ='<span class="badge badge-danger">Rechazado</span>';
							break;
						default:
							htmlStatus = '<span class="badge badge-secondary">Estado desconocido</span>';
							break;
					}
...
function fntAprobarProceso(idProcesoAdopcion){
	let request = (window.XMLHttpRequest) ? XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP')
	let ajaxUrl = BaseUrl+'/Donaciones/consultarAdopcion/'+idProcesoAdopcion;
	...
	request.onreadystatechange = function(){
		if(request.readyState == 4 && request.status == 200){
			...
			if(objData.status)
			{	
				document.querySelector("#idProcesoAdopcion").value = objData.data.idProcesoAdopcion;
				document.querySelector("#txtDescripcion").value = objData.data.requisitos;
				document.querySelector("#idAnimalProceso").value = objData.data.idAnimal;
				if(objData.data.requisitos){
					tinymce.activeEditor.setContent(objData.data.requisitos);
				}
				$('#modalProcesoAdopcion').modal('show');
